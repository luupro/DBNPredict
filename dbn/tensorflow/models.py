...
class TensorFlowAbstractSupervisedDBN(BaseAbstractSupervisedDBN, BaseTensorFlowModel):
    ...
    def _initialize_weights_for_1_MPL(self):

        if self.unsupervised_dbn.activation_function == 'sigmoid':
            stddev = 1.0 / np.sqrt(self.input_units)
            self.W_1 = tf.Variable(tf.random_normal([self.input_units, self.input_units], stddev), name='W_1')
            self.b_1 = tf.Variable(tf.random_normal([self.input_units], stddev), name='b_1')
            self.W_2 = tf.Variable(tf.random_normal([self.input_units, self.num_classes], stddev), name='W_2')
            self.b_2 = tf.Variable(tf.random_normal([self.num_classes], stddev), name='b_2')
            self._activation_function_class = tf.nn.sigmoid
        elif self.unsupervised_dbn.activation_function == 'relu':
            stddev = 0.1 / np.sqrt(self.input_units)
            self.W_1 = tf.Variable(tf.truncated_normal([self.input_units, self.input_units], stddev), name='W_1')
            self.b_1 = tf.Variable(tf.constant(stddev, shape=[self.input_units], dtype=tf.float32), name='b_1')
            self.W_2 = tf.Variable(tf.truncated_normal([self.input_units, self.num_classes], stddev), name="W_2")
            self.b_2 = tf.Variable(tf.constant(stddev, shape=[self.num_classes], dtype=tf.float32), name='b_2')
            self._activation_function_class = tf.nn.relu
        else:
            raise ValueError("Invalid activation function.")
    ...
    def _build_model(self, weights=None):
        ...
        # weights and biases
        if self.n_hidden_layers_mlp == 0:
            self._initialize_weights(weights)
        elif self.n_hidden_layers_mlp == 1:
            self._initialize_weights_for_1_MPL()
        else:
            raise ValueError("number of layer MLP is 0 or 1")
        ...
        if self.n_hidden_layers_mlp == 0:
            ...
            hidden_layer_1 = tf.matmul(self.transform_op, self.W_1) + self.b_1
            hidden_layer_1 = tf.nn.relu(hidden_layer_1)
            # Dropout on hidden layer
            #self.keep_prob_mlp = tf.placeholder(tf.float32)
            #hidden_layer_1_dropout = tf.nn.dropout(hidden_layer_1, self.keep_prob_mlp)
            self.y = tf.matmul(hidden_layer_1, self.W_2) + self.b_2
    ...
    def _fine_tuning(self, data, _labels):
        ...
        if self.n_hidden_layers_mlp == 0:
            ...
            sess.run(tf.variables_initializer([self.W_1, self.W_2, self.b_1, self.b_2]))
        else:
            raise ValueError('Fine tunning just for number of MLP is 0 or 1')
        ...
        #tf.train.Saver().save(sess, '../saved_model/lorenz')
...
class SupervisedDBNRegression(TensorFlowAbstractSupervisedDBN, RegressorMixin):
    ...
    def _build_model(self, weights=None):
        ...
        if self.cost_function_name == 'mse':
            self.cost_function = tf.reduce_mean(tf.square(self.y_ - self.y), name='cost')  # Mean Squared Error
        elif self.cost_function_name == 'mae':
            self.cost_function = tf.reduce_mean(tf.abs(self.y_ - self.y), name='cost')
        elif self.cost_function_name == 'mape':
            self.cost_function = tf.reduce_mean(tf.abs(tf.divide(tf.subtract(self.y, self.y_), self.y_)), name='cost')
        else:
            raise ValueError('invalid cost function name')
