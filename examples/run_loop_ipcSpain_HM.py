...
from RandomRegressionCpi import RandomRegressionCpi
...
for i in range(0, 2000):
    print('Run index: %f' % i)
    RandomRegressionCpi.number_visible_input = randint(1, 10)
    RandomRegressionCpi.number_hidden_input = randint(1, 10)
    data_train, label_train, data_test, label_test = \
        HSMemory.create_train_and_test_data(lorenz_scale.tolist(), RandomRegressionCpi.number_visible_input)
    print("Shape of data_train: " + str(data_train.shape))
    print("shape of label_train: " + str(label_train.shape))

    start_time = time.time()
    regressor, tmp_lrr, tmp_lr = RandomRegressionCpi.create_random_model()
    regressor.fit(data_train, label_train)
    tmp_train_mse = sum(regressor.train_loss) / RandomRegressionCpi.number_iter_backprop
    ...
    del regressor
    tmp_element_data = [tmp_lrr, tmp_lr, RandomRegressionCpi.number_visible_input, RandomRegressionCpi.number_hidden_input,
...
#workbook = xlsxwriter.Workbook('result_vdmt_' + dt_string + '.xlsx')
workbook = xlsxwriter.Workbook('result_ipcSpain_' + dt_string + '.xlsx')
