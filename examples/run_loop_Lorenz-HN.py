...
from RandomRegressionLorenz import RandomRegressionLorenz
...
for i in range(0, 2000):
    print('Run index: %f' % i)
    RandomRegressionLorenz.number_visible_input = randint(1, 10)
    RandomRegressionLorenz.number_hidden_input = randint(1, 10)
    data_train, label_train, data_test, label_test = \
        HSMemory.create_train_and_test_data(lorenz_scale.tolist(), RandomRegressionLorenz.number_visible_input)
    print("Shape of data_train: " + str(data_train.shape))
    print("shape of label_train: " + str(label_train.shape))

    start_time = time.time()
    regressor, tmp_lrr, tmp_lr = RandomRegressionLorenz.create_random_model()
    regressor.fit(data_train, label_train)
    tmp_train_mse = sum(regressor.train_loss) / RandomRegressionLorenz.number_iter_backprop
    ...
    del regressor
    tmp_element_data = [tmp_lrr, tmp_lr, RandomRegressionLorenz.number_visible_input, RandomRegressionLorenz.number_hidden_input,
...
workbook = xlsxwriter.Workbook('result_loop_Lorenz_' + dt_string + '.xlsx')
